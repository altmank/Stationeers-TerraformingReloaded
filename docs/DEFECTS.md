...
| D17 | **Found by review.** A world's storms carry fixed temperature offsets sized for the world as shipped (Vulcan's ash storm is -275 K by day). On a Vulcan cooled to 293 K that reads 18 K and freezes the whole atmosphere in one tick, and the caps give it back too slowly to recover. | `GlobalGasMix.cs:553-556`; `S/Data/weather.xml` | `Climate.StormInProportion`: on a world the mod fills in, a storm's offset shrinks in the proportion the planet has cooled; never enlarged, untouched on untouched air. Model: a finished Vulcan reads 231 K in an ash storm and keeps its air | Not run live (no way yet to force a storm headless) |

...
| --- | --- | --- |
| `tools/PatchCheck` (run by `build.ps1`) | Required targets exist, each tank method asks the question exactly once, the rewritten IL runs (it calls the rewritten `AddEnergy` and `RemoveEnergy`). The trace gas transpiler is applied on its own and must rewrite the lerp's one take and compile; `TraceGases.CheckArithmetic` must show the planet paying for every mole it moves, the per-cell, per-tick and planet bounds holding, and a gas above the line left alone. Mutation-checked (0.11.0): dropping the clamp to what the planet holds, or not charging the tick's budget, fails it | Anything reaching Unity native code: outside Unity the CLR refuses to compile it ("ECall methods must be packaged into a system module"). The save guard is one; it reports `bound` and the extras are then not attempted |
...
| `-Sidecar` | Per-world settings. Two phases: make a world, then load it and rewrite its settings file one shape at a time, calling the mod's own read path on each, so a real file over the real code is what is judged. The config asks for a 500 kPa ceiling throughout and every line prints it, so a run where the config was not asking cannot read as a pass. Nineteen cases: a ceiling of 0, -5, 99999999 and -INF; every field -5; NaN and INF scales; a zero half-life; a nil half-life; one field only; no file; not XML; no station name; a folder that moved; version 99; and a read after the sidecar has stood down | The client branch, which cannot be faked headlessly |
| `-Upgrade -OldRoot <checkout>` | A world made by the previous release, loaded by this one. Builds the mod in a git worktree of the earlier tag, makes Mars with it under a tuned config (greenhouse 1.7, heat limit 33, Custom planet size 0.02, no ceiling) and saves; then sets the config's ceiling to 0.5 kPa, the damage case, and loads the world with this build. It must take everything but the ceiling from the config, refuse the ceiling, write its own settings file, keep the typed custom size, and hold the planet's total at what was saved. `-Control` loads with the earlier release again instead; `-UpgradeCeiling 0` leaves the ceiling alone | A real player's save, with a base in it |
| `-RainSave` | Rain on a world that ships no weather (Mimas), across a save and a load. A cloud is filled, the planet tick empties it into the air and schedules rain for real, the world is saved, then loaded in a fresh instance. The game saves no weather state on such a world (`WeatherManager.CreateSaveData` returns null), so the rain is expected to be gone after the load; what is judged is the planet's whole store, tank plus the three reservoirs | Rain that is already falling at the save, rather than scheduled |
| `-Sessions` | Several worlds in one game session, switched with the console's own `file start`. World A is changed with `terraform set` (lowering the heat limit must ask first; a value that is not on or off and an unknown key must be refused), then the config is changed through the BepInEx entry, the way the config editor does it. World B is created and must start from the new config; A, loaded again, must still have what `terraform set` gave it; B, loaded again, must still have what it started with. Values in force and values in the file are both read at every step. The mod's config file is restored byte for byte afterwards | That the in-game config editor fires the same `SettingChanged` event the driver does |
...
- `-Sidecar`, first runs, 2026-09-22. **Through all nineteen hostile files the planet held
  ...
  **The control is what makes that mean something**: `terraform set MaxPressureKPa 0.5 confirm`, the one path by
...
- `-Upgrade` from `v0.9.1`, 2026-09-22. The world 0.9.1 made and saved held **1,011,899.956 mol at the
  save and 1,011,899.956 mol 60 ticks after 0.10.0 loaded it**. With the config asking for a 0.5 kPa
  ceiling, the loaded world ran with none, at 2.908 kPa, and wrote `nil` into its new settings file;
  greenhouse 1.7, heat limit 33 and the 0.02 custom size came through from the 0.9.1 config, and the new
  storm settings took their defaults. **The control is what makes that mean something**: the same world
  loaded by 0.9.1 took the 0.5 kPa ceiling and held 162,104 mol at tick 60, about 84 % of its air
  deleted. Loaded outdoor cells read about 3,670 mol high at tick 5 and settle by tick 60, identically
  under 0.9.1 and 0.10.0, so it is not the upgrade's doing; the figure that matters is against the save.
- `-RainSave`, 2026-09-22, Mimas. The planet tick scheduled rain on a world that ships no weather, the
  save dropped it as the game does on such a world, and the planet's store was **8,000.000 mol after
  the save and 8,000.000 mol after the load**. Nothing is lost but the rest of that one rain.

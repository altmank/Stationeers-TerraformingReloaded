...
to the model's side; what is compared is everything else: base, sun distance, the mod's greenhouse and
density rule, and whatever the world ships itself.
...
def main():
    ...
    if not rows:
        ...
        return 1
    worst, shown = 0.0, 0
    print('%6s %7s %7s %9s %9s %7s  %s' % ('tick', 'angle', 'orbit%', 'game K', 'model K', 'gap', 'model says would leave the air'))
    for i, m in enumerate(rows):
        if m.group('weather') not in ('None', 'SnowScheduled', 'RainScheduled'):
            continue                                   # a running weather event adds its own offset
        p = Planet(world)
        p.air = {}
        ...
        model = p.temperature(float(m.group('angle')), float(m.group('orbit')))
        ...
        if i % max(1, len(rows) // 12) == 0 or abs(gap) > tolerance:
            ...
            print('%6s %7.1f %7.1f %9.2f %9.2f %+7.2f  %s' % (m.group('tick'), float(m.group('angle')), float(m.group('orbit')), game, model, gap, leaving[:90]))

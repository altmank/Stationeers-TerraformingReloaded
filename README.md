...
- **Venus and Vulcan** start hot because of the air they start with. The mod reads that heat as
  ...
  evens out Vulcan's day and night. They are removal projects: strip first, then add air.
...
heat to get started, Europa nine times. Venus and Vulcan are mostly removal, which is slower per mole
than adding.
...
planet keeps the size it has; `terraform` shows it.
...
also in `BepInEx/config/xceled.stationeers.terraformingreloaded.cfg`. All apply at once except the
two marked.
...
| `MaxPressureKPa` | 0 | Planet pressure ceiling. 0 is none |
...
| `MaxExternalOffsetKelvin` | 50 | Most that added heat can shift the planet |
...
| `tools/LiveCheck` | Headless in-game conservation tests |
...
| `tools/Balance` | Pacing model: hours to terraform each world per base size, and the planet size that gives a target |
